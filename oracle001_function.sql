...
sunder       Abel
sunder       Ande
-- last_name이 다르면 중복된 것이 아님 fist last_name 둘다 같아야 함
*/
...
--empoyees테이블에서 first_name컬럼의 값이 'David' 일때의
--first_name, salary을 출력하라
SELECT first_name, salary
FROM employees
WHERE first_name = 'David';--실제값은 대소문자 구별 , where절에서 같다는 =사용 
...
FROM employees
WHERE salary < 3000;
--employees 테이블에서 first_name칼럼의 값이'David' 아닐떄의
--first_name, salary을 출력하라.
--1)
SELECT first_name, salary
FROM employees
...
--employees테이블에서 salary가 3000, 9000, 17000 아닐때
--first_name, hire_date, salary을 출력하라.
--1)
SELECT first_name, hire_date, salary
FROM employees
...
--employees테이블에서 commision_pct이 null이 아닐때
--first_name, salary commission_pct을 출력하라.
SELECT first_name, salary, commission_pct
FROM employees
WHERE commission_pct IS NOt NULL;
--employees테이블에서 first_name에 'dar'이 포함이된
--first_name, salary, email을 출력하라
SELECT first_name, salary, email
FROM employees
WHERE first_name LIKE '%der%';

--ememployees테이블에 first_name의 값중 'A'로 시작하고
--두번째 문자는 임의 문자이면 'exander'로 끝나는
--firs_name, salary, email을 출력하라
SELECT first_name, salary, email
FROM employees
...
--ememployees테이블에서 job_id을 오름차순으로
...
--ememployees테이블에서 department_id을 오름차순으로
--first_name을 내림차순으로
...
--ememployees테이블에서 업무(job_id)이 'FI_ACCOUNT'인 사람들의
