...
--substr('문자', 시작위치, 개수)
SELECT substr('oralce test', 1,4) /* substring(start, end)*/
FROM dual;

SELECT substr('oralce test', -3,4) 
FROM dual;
...
/*---------------------------------------------------------
일반함수(NULL)
--2. nvl2(칼럼, 대체값1, 대체값2) : 칼럼의 값이 null아니면 대체값1, null이면 대체값2로 출력한다.
-----------------------------------------------------------*/
...
일반함수(NULL)
3. NULLIF(칼럼, 비교값)
   칼럼값고 비굣값이 같으면 NULL로 리턴하고 같지 않으면 컬럼의 값으로 리턴한다.
-------------------------------------------------------------*/
...
일반함수(NULL)
4. coalesce(컬럼1, 컬럼2)
   -칼럼1, 컬럼2 모두 Null이 아니면 컬럼1을 리턴한다.
...
-- 직급이 'PE_REF'인 사원은 5%, 'SA_MAN'인 사원은 10%,
-- 'AC_MGR'인 사원은 15%, 'PU_CLERK'인 사원은 20%를 인상
SELECT job_id, salary, decode(job_id, 'PE_REF', salary *1.05,
'SA_MAN', salary *1.1,
'AC_MGR', salary *1.15,
...
-- salary값이 10000미만이면 '하', 10000이상 20000미만이면 '중', 20000이상이면 '상'
...
max([DISTIMCT] | [ALL] 칼럼): 최대값
min([DISTIMCT] | [ALL] 칼럼): 최소값
count([DISTIMCT] | [ALL] 칼럼): 개수
sum([DISTIMCT] | [ALL] 칼럼): 합계
avg([DISTIMCT] | [ALL] 칼럼): 평균
stddev([DISTIMCT] | [ALL] 칼럼): 표준편차
variance([DISTIMCT] | [ALL] 칼럼): 분산
--------------------------------------*/
...
GROUP BY department_id
HAVING department_id <=50 OR department_id IS NULL  /*NULL은 비교연산자를 사용 할 수 없다 */
ORDER BY department_id;

--부서별의 직원수가 5이하인 경우만 출력하시오.
SELECT department_id, count(*) AS "인원수" --4
FROM employees --1
GROUP BY department_id --2
HAVING count(*) <=5 --3
ORDER BY department_id;--5번
--자바와 연동하기 위해 한글참조 사용하지 말것 나중에 문제됨
...
--부서별 최소급여, 최대급여를 출력하는데 최소급여와 최대급여가 같은 경우에만 부서별 오름차순으로 출력하시오.
